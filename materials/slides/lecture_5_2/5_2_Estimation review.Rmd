...
With i.i.d. observations, these imply the joint conditional density ($\phi$ is the normal PDF)

$$f(Y_1,\dots\,Y_N|X_1,\dots\,X_N,\beta,\sigma^2) = \prod_{i=1}^N \phi(Y_i-X_i\beta,\sigma^2)$$
...
where $S_0 = E[g(z_i; \theta_0) g(z_i; \theta_0)^\prime]$ it the variance-covariance matrix of moments (an $M\times M$ matrix)
...
$$Var(\hat{\theta}_{GMM}) = (D_0^\prime W_0 D_0)^{-1}$$
---

## Computing $\hat{\theta}_{GMM}$

<!-- - We need W. How to calculate W? 2-step GMM -->

<!-- ## Computing $Var(\hat{\theta}_{GMM})$ -->

<!-- ## Finite sample VCOV with bootstrapping -->
