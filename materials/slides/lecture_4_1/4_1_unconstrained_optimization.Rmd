...
.footnote[\*These slides are based on Miranda & Fackler (2002), Nocedal & Wright (2006), Judd (1998), and course materials by Ivan Rudik.]
...
- If $n = 1$, this means that $f^{\prime \prime}(x^*) \leq 0$, i.e., the function is locally convex
...
> Suppose $\nabla^2 f$ is continuous in an open neighborhood of $x^*$ and that $\nabla f(x^*) = 0$ and $\nabla^2 f(x^*) = 0$ is positive definite. Then $x^*$ is a strict local minimizer of $f$
...
Nelder-Mead is a pain to code efficiently (i.e. don't spend the time doing it yourself)
...
We can check the solution using `Optim.minimizer`

```{julia}
Optim.minimizer(soln)
```
...
- Nelder-Mead is commonly used but slow and unreliable
  - It has no real useful convergence properties

- Unless you're solving a problem with derivatives that are costly to calculate or approximate, avoid using Nelder-Mead
