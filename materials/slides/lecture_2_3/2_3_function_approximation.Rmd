...
2. **Numerical operations and representations** 
   ...
   3. **Function approximation** $\leftarrow$ .blue[We're back here]
...
n = 5;
coefs = [0.1, 0.2, 0.0, 0.1, -0.1];
# Note that this package uses domain in the reverse order
domain = [10.0, 0.0]; 
my_cheb_poly = ChebPoly(coefs, n, domain)
```
...
```{julia}
chebyshev_evaluate(my_cheb_poly, [3.0])
```
...
**Key idea**. *Instead looking $f$ in an infinite-dimensional function space, we look for its approximant $\hat{f}$ in a finite-dimensional space*

---

## Numerically solving functional equations

Once again, we use approximats with a linear combination of basis functions
...
$$
- $g(x, \phi(x)c)$ can be regarded as *residual*, which we want to make as close to zero as possible for a good approximation
